...
.SYNOPSIS
  ...
  ScriptName: InitialToastNotification.ps1

.DESCRIPTION
  
  Simple script to show a Toast Notification to end-user to signify that the device is still being setup, post provisioning. 
  The script stays up on the screen for a specified duration unless users clicks on it post which it closes.

  Original credit to Trevor Jones for the script whcih can be found here at https://smsagent.blog/2018/02/01/create-a-custom-toast-notification-with-wpf-and-powershell/
...
$Text = "Hi $Env:Username, your device is being setup. Will notifiy again when the device is ready."
$Timeout = 1800
